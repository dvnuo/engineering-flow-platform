...
def test_runtime_chat_enables_member_memory_for_interactive_chats_only(monkeypatch):
    class _FakeConfig:
        @property
        def session(self):
            return {}
    ...
    profile_off = runtime_chat._runtime_config(
        ...
        track_usage=True,
        interactive=True,
        runtime_profile_config={"enable_member_memory": False},
    )

    assert interactive.enable_member_memory is True
    assert interactive.member_memory_dir is None
    assert background.enable_member_memory is False
    assert profile_off.enable_member_memory is False
