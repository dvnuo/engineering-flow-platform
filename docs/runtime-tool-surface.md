...
| Tools: session_search | Conditional | Registered when `enable_session_search` is set, which the gateway does for every interactive chat (a profile can set it to `false`); background tasks and sub-agents never see it. The tool searches and reads this assistant's other stored sessions in the same session root, indexing member and assistant text only (no tool output or reasoning). `scope=mine` (default) keeps sessions whose member turns carry the Portal member's `author_id`; `scope=agent` covers every session, matching what Portal already shows for the agent. The member is the header-derived `portal_user_id`; without it `mine` matches nothing, and reading a session that is not the member's own needs an explicit `scope=agent`, so a run that lost its identity never shows another member's sessions by default. When the tool is offered, the system prompt gains an "Earlier sessions" block listing the member's recent sessions (header listings are reused for 30 seconds per session root). |
