...
- The read-only troubleshooting CLIs `nexus`, `splunk`, `appd`, and `pgsql` are configured through the profile sections of the same names. `nexus`/`splunk`/`appd` are projected like jira (`EFP_NEXUS_INSTANCES_0_BASE_URL`, `EFP_SPLUNK_INSTANCES_0_AUTH_TOKEN`, `EFP_APPD_INSTANCES_0_ACCOUNT`, `EFP_APPD_INSTANCES_0_AUTH_TYPE=api_client`, ...) with an empty `rest_path` because each CLI owns its REST prefix; `pgsql` is copied verbatim (`EFP_PGSQL_INSTANCES_0_HOST`, `_DATABASE`, `_USERNAME`, `_PASSWORD`, `_SSLMODE`). The CLI instruction text names all four when any is enabled.
