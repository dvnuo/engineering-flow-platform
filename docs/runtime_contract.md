...
- The troubleshooting CLIs `nexus`, `splunk`, `appd`, and `pgsql` are configured through the profile sections of the same names. `nexus`, `splunk` and `appd` are read-only binaries; `pgsql` runs whatever the configured database role and endpoint allow, so a profile that should only read is given a read-only role or a replica endpoint. `nexus`/`splunk`/`appd` are projected like jira (`EFP_NEXUS_INSTANCES_0_BASE_URL`, `EFP_SPLUNK_INSTANCES_0_AUTH_TOKEN`, `EFP_APPD_INSTANCES_0_ACCOUNT`, `EFP_APPD_INSTANCES_0_AUTH_TYPE=api_client`, ...) with an empty `rest_path` because each CLI owns its REST prefix; `pgsql` is copied verbatim (`EFP_PGSQL_INSTANCES_0_HOST`, `_DATABASE`, `_USERNAME`, `_PASSWORD`, `_SSLMODE`). The CLI instruction text names all four when any is enabled.
- `bash` output is passed through the runtime's secret redaction before it reaches the model and before the full output is archived under `.efp_runtime/tool-output/`; `bash` also exposes the command string as its permission subject, so `tool_permissions.bash` accepts subject globs such as `{"*kubectl*delete*": "deny", "*": "allow"}`.
