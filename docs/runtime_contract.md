...
- The troubleshooting CLIs `nexus`, `splunk`, and `pgsql` are configured through the profile sections of the same names. `nexus` and `splunk` are read-only binaries; `pgsql` runs whatever the configured database role and endpoint allow, so a profile that should only read is given a read-only role or a replica endpoint. `nexus`/`splunk` are projected like jira (`EFP_NEXUS_INSTANCES_0_BASE_URL`, `EFP_SPLUNK_INSTANCES_0_AUTH_TOKEN`, `EFP_SPLUNK_INSTANCES_0_APP`, ...) with an empty `rest_path` because each CLI owns its REST prefix; `pgsql` is copied verbatim (`EFP_PGSQL_INSTANCES_0_HOST`, `_DATABASE`, `_USERNAME`, `_PASSWORD`, `_SSLMODE`). The CLI instruction text names all three when any is enabled.
